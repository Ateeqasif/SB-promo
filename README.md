...
- Headings: real brand font is **Neue Regrade** (ExtraBold 800), a paid/commercial font not on Google Fonts — confirmed via slickblinds.com's own DevTools ("Rendered Fonts" shows it loads successfully as a network resource; its CSS lists Montserrat further down the stack but that's only a technical fallback for a failed load, and doesn't visually resemble it). **Manrope** ExtraBold is used instead as the closest freely-licensed match — geometric, rounded terminals, tight tracking — chosen against the official brand reference PDF early in this project. Add Neue Regrade via `@font-face` and update `--font-head` in `css/style.css` if you get a license.
